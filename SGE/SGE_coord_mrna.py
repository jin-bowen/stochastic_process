...
def main():
	...
	for params, color in zip(params_list, color_list):
		...
		fig.suptitle('bursting expression pattern\ncoordinated biallelic expression')
		np.random.seed(3442)
	
		for i in range(1,N):
			pop_ref, t = gillespie_algorithm(params, hazard, S, pop_0, T=100, sample_point=sample_point)
			pop_alt, t = gillespie_algorithm(params, hazard, S, pop_0, T=100, sample_point=sample_point)
		...
		plt.savefig('param%s_coord_SGE.png'%str(ii))
		ii += 1
